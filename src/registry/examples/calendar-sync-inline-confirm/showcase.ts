...
export default defineShowcase({
  ...
  preview: 'fit',
  description:
    'A white "Calendar" settings pill whose "Sync Events" button confirms in place — no toast, no spinner beside it. Click and the label blurs away, the button darkens into a track with a white bar sweeping left→right (indeterminate), the bar snaps full, then the whole track collapses width-wise into a gradient black dot as a check springs in. It auto-resets so you can fire it again. All feedback happens inside the button’s own footprint.',
...
<button
  ...
  <span className={phase === 'idle' ? 'opacity-100' : 'opacity-0 blur-[6px]'}>Sync Events</span>
